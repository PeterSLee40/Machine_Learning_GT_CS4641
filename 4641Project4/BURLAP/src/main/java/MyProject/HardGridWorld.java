...
import MyProject.util.AgentPainter;
import MyProject.util.AtLocation;
import MyProject.util.HolePainter;
import MyProject.util.LineManPainter;
import MyProject.util.LocationPainter;
import MyProject.util.Movement;
import MyProject.util.TopPainter;
import MyProject.util.WallPainter;
...
public class HardGridWorld implements DomainGenerator {
	...
	public HardGridWorld(int[][] map,int mapx, int mapy){
		this.map = map;
		this.mapx = mapx;
		this.mapy = mapy;
	...
	public static State createStartState(Domain domain, int x, int y) {
		State s = new MutableState();
		ObjectInstance agent = new MutableObjectInstance(
				domain.getObjectClass(CLASSAGENT), "agent0");
	...
	public StateRenderLayer getStateRenderLayer() {
		...
		rl.addStaticPainter(new TopPainter(map));
		rl.addStaticPainter(new HolePainter(map));
